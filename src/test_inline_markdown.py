...
# Test splitting nodes by delimiter
    ...
    def test_text_to_textnode_alt_order(self):
        text = "This is an ![obi wan image](https://i.imgur.com/fJRm4Vk.jpeg) and a [link](https://boot.dev) with **text** with an _italic_ word and a `code block`"
        new_nodes = text_to_textnodes(text)
        self.assertListEqual(
        [
